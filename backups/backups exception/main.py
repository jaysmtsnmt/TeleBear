...
'''
    try: 
        ...
        print("[TELE] Start Message Timeout...")
        ...
        try:    
            ...
            if payload != None:
                if len(register_cache.get(f"{update.effective_user.id}")) == 1: #completed stage 1, username, saving
                    ...
                    try:
                        user_instance.completeGoogleAPIAuth(flow)
                    ...
                    if query[2] == "thisweek":
                        ...
                        if len(query) == 3:
                            ...
                            if not os.path.exists(path): #if existing week does not already exist, load from portal
                                ...
                                try:
                                    ...
                                    m = "You are not registed! Please start by using /register!"
                                    context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                                    return False
                                ...
                                try:
                                    ...
                                    path = agent.getTimetable(str(t_monday))
                                
                                except Vortal.IncorrectLoginInformation:
                                    m = "Your VJC Portal Login Information is incorrect. Please update them by using /portal!"
                                    context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                                    return False
                                
                                except Vortal.PermissionError:
                                    m = "An unexpected error occured... Internal Cache Permission Error!"
                                    context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                                    return False
                                
                                except Vortal.Requests.AccessDenied:
                                    m = "An unexpected error occured... Internal Requests Access Denied!"
                                    context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                                    return False
                                
                                except Vortal.Timetable.MissingLessonID:
                                    m = "An unexpected error occured... Internal Timetable Missing Lesson ID"
                                    context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                                    return False
                            ...
                            try:
                                ...
                                m = "An unexpected error occured... Missing Lesson ID L499"
                        ...
                        if len(query) == 3:
                            ...
                            if not os.path.exists(path): #if existing week does not already exist, load from portal
                                ...
                                try:
                                    agent = portalAgent(username, password, update.effective_user.id)
                                    path = agent.getTimetable(str(n_monday))
                ...
                if query[1] == "save": #enter save menu
                    try: 
                        if query[2] == "thisweek":
                            ...
                            total = bearlander_instance.save_to_gcal(0)
                            ...
                            bearlander_instance = bearlander(update.effective_user.id)
                            total = bearlander_instance.save_to_gcal(1)
                        ...
                        m = "The API Authentication link has expired. Please try again with /google!"
                        context.bot.send_message(chat_id=update.effective_chat.id, text=m)
                        return False
