...
redirect_uri = "https://fd99-116-15-77-104.ngrok-free.app"

print(f"[USER] Confirm Redirect URI: {redirect_uri} ")
...
class User():
    ...
    def getVortalInformation(self):
        ...
        if os.path.exists(path):
            ...
            raise UserException.Cache.UserDoesNotExist(f"{self.user} information does not exist.")
        ...
        if creds == None or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                if not authed:
                    ...
                    raise UserException.GoogleAPI.AuthenticationTimeout("getGoogleAPICredentials Authentication timeout. ")
    ...
    def completeGoogleAPIAuth(self, flow: Flow.from_client_secrets_file):
        ...
        if not authed:
            print("[DATA] Authentication Timeout.")
            raise UserException.GoogleAPI.AuthenticationTimeout("completeGoogleAPIAuth Authentication timeout. ")
